...
const Todos = () => {
  ...
  const API_URL = import.meta.env.VITE_REACT_APP_API_URL; // url rakhcha

  // Fetch todos from the backend when the component mounts
  useEffect(() => {
    fetch(`${API_URL}/todos`)
      .then((response) => response.json()) //binary to json conversion
      .then((data) => {
        setTodos(data);
      })
  ...
  const handleCreateTodo = (e) => {
    ...
    fetch(`${API_URL}/todos`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",   //send the request to the backend
      },
